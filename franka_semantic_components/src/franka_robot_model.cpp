...
void FrankaRobotModel::initialize() {
  ...
  if (franka_state_interface != state_interfaces_.end() &&
    ...
    robot_model = bit_cast<franka_hardware::Model*>((*franka_model_interface).get().get_optional().value());
    robot_state = bit_cast<franka::RobotState*>((*franka_state_interface).get().get_optional().value());
  } else {
